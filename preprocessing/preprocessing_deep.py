...
from data import PATH_DWIE_DATA, PATH_DWIE_NER_FLAIR

DWIE_NER_TYPES = pd.read_csv("/home/sylvain/POPCORN/DWIE/data/data_DWIE_official_splits_FR/onto_deep_dwie.csv", sep=",")
...
def spacy_to_flair_doc(doc) -> list:
    ...
    for i, sent in enumerate(doc.sents):
        if '\tfooter' not in spacy_to_flair_sent(sent):
...
if __name__ == "__main__":
    ...
    params = vars(args)
    DWIE_NER_TYPES = DWIE_NER_TYPES[DWIE_NER_TYPES["Deep"] <= int(params["deep_ontologie"])]
    
    DWIE_NER_TYPES = DWIE_NER_TYPES.set_index('Labels')['Deep'].to_dict()
    main()